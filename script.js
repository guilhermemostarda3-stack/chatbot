    ...
    <script>
      ...
      function showUserData() {
        if (userData) {
          ...
          profilePassword.textContent = "•".repeat(userData.password.length); // Esconde a senha
      ...
      function createMessageElement(messageData) {
        ...
        if (isImage) {
          ...
          title.innerHTML = `**Aqui estão alguns sites para encontrar imagens sobre o seu tema:**`;
      ...
      async function searchForGif(searchTerm, botMessageId) {
        try {
          ...
          if (data.data && data.data.length > 0) {
            ...
            messageToUpdate.text = `Desculpe, não encontrei nenhum gif para "${searchTerm}".`;
          }
        } catch (error) {
